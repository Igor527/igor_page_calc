  ...
  try {
    ...
    if (!isAdminUser(cred.user)) {
      ...
      return {
        ok: false,
        error: email
          ? `Этот аккаунт Google не в списке админов. Добавьте в .env: VITE_ADMIN_EMAIL=${email}`
          : 'Этот аккаунт не в списке админов. Добавьте ваш email в VITE_ADMIN_EMAIL в .env.',
      };
    }
    return { ok: true };
  } catch (e: unknown) {
  ...
  try {
    ...
    if (!isAdminUser(cred.user)) {
      ...
      return {
        ok: false,
        error: ghUid
          ? `Этот аккаунт GitHub не в списке админов. Добавьте в .env: VITE_ADMIN_EMAIL=ваш@email или VITE_ADMIN_GITHUB_IDS=${ghUid}`
          : 'Этот аккаунт GitHub не в списке админов. Добавьте VITE_ADMIN_EMAIL или VITE_ADMIN_GITHUB_IDS в .env.',
      };
    }
    return { ok: true };
  } catch (e: unknown) {
    ...
    if (!isAdminUser(cred.user)) {
      ...
      if (isGoogle) {
        return {
          ok: false,
          error: email
            ? `Этот аккаунт Google не в списке. Добавьте в .env: VITE_ADMIN_EMAIL=${email}`
            : 'Добавьте ваш Gmail в VITE_ADMIN_EMAIL в .env.',
          provider: 'google',
        };
      }
      return {
        ok: false,
        error: ghUid
          ? `Этот аккаунт GitHub не в списке. В .env добавьте: VITE_ADMIN_GITHUB_IDS=${ghUid} или VITE_ADMIN_EMAIL=${email || 'email@из.github'}`
          : `Добавьте в .env VITE_ADMIN_EMAIL или VITE_ADMIN_GITHUB_IDS.${email ? ` Email этого аккаунта: ${email}` : ''}`,
        provider: 'github',
      };
    }
    return { ok: true };
...
export async function signIn(email: string, password: string): Promise<{ ok: boolean; error?: string }> {
  ...
  try {
    ...
    if (!isAdminUser(cred.user)) {
      await firebaseSignOut(a);
      return { ok: false, error: 'Этот аккаунт не имеет прав админа.' };
    }
    return { ok: true };
  } catch (e: unknown) {
