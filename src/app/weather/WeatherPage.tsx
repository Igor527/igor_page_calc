...
const WeatherPage: React.FC = () => {
  ...
  const showWizard = data.length === 0 && !loading;
  const wizardSteps = useMemo(
    () => [
      {
        title: 'Публикация CSV',
        summary: 'В Google Таблице включите «Публикация в интернете» в формате CSV — обычного «Открыть доступ» недостаточно.',
        content: (
          <ol style={{ margin: 0, paddingLeft: 20, fontSize: 13, lineHeight: 1.5 }}>
            <li>Файл → <strong>Публикация в интернете</strong></li>
            <li>Выберите лист и формат <strong>CSV</strong></li>
            <li>Скопируйте ссылку вида <code>.../output=csv</code></li>
          </ol>
        ),
        more: (
          <p style={{ margin: 0, fontSize: 12 }}>
            Колонки: дата/время, температура, давление, pm1, pm2.5, pm10, станция. Либо задайте{' '}
            <code>VITE_WEATHER_SHEET_CSV_URL</code> и Firebase в <code>.env</code> для автозагрузки.
          </p>
        ),
      },
      {
        title: 'URL и загрузка',
        summary: 'Вставьте опубликованную ссылку CSV и нажмите «Обновить».',
        content: (
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8, alignItems: 'center' }}>
            {!getWeatherFirebaseConfig() && (
              <input
                ...
                placeholder="URL CSV из Google Таблицы"
                style={{
                  flex: '1 1 200px',
                  minWidth: 200,
                  padding: '8px 12px',
        ...
        more: getWeatherFirebaseConfig() ? (
          <p style={{ margin: 0, fontSize: 12 }}>Настроен Firebase — данные подтягиваются автоматически при открытии страницы.</p>
        ) : (
          <p style={{ margin: 0, fontSize: 12 }}>
            <strong>CORS:</strong> если браузер блокирует запрос, откройте ссылку CSV в новой вкладке, скопируйте текст (Ctrl+A) и вставьте на шаге 3.
          </p>
        ),
      },
      {
        title: 'Ручная вставка',
        summary: 'Если по ссылке не грузится — вставьте содержимое CSV вручную.',
        content: (
          <>
            <textarea
              value={pasteCsv}
            ...
            <button
              ...
              className="primary"
              style={{ marginTop: 8, padding: '8px 16px', fontSize: 13, minHeight: 44 }}
            >
              Применить
            </button>
          </>
    ...
    [csvUrl, loading, pasteCsv, load, loadFromFirebase]
  );
  ...
  return (
    <div style={{ maxWidth: 900, margin: '0 auto', padding: '24px 20px' }}>
      ...
      {error && (
        <p style={{ color: 'var(--pico-del-color)', marginBottom: 16, fontSize: 14 }} role="alert">{error}</p>
      )}

      {showWizard && error && (
        <details style={{ marginBottom: 16, fontSize: 13 }} open>
          <summary style={{ cursor: 'pointer', color: 'var(--pico-muted-color)' }}>Подробнее об ошибке</summary>
