...
| `@id.stepsCalculations` | Формула с подставленными числами (без round) | `@totalCost.stepsCalculations` → `50 * 100` |
| `@id:exprOnly` | То же, что `.stepsCalculations` | `@totalCost:exprOnly` → `50 * 100` |
...
1. Ячейка формулы: `@id.stepsCalculations` (показывает выражение с числами)
